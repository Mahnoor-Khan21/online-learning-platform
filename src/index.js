...
// Keep the platform limited to ONE administrator account.
// If old data contains multiple admins, keep ADMIN_EMAIL (when configured),
// otherwise keep the oldest admin and safely demote the others to teachers.
async function enforceSingleAdmin() {
    const configuredAdminEmail = String(process.env.ADMIN_EMAIL || '').trim().toLowerCase();
    const admins = await User.find({ role: 'admin' }).sort({ createdAt: 1, _id: 1 });

    if (admins.length <= 1) return;

    let primaryAdmin = configuredAdminEmail
        ? admins.find(user => String(user.email).toLowerCase() === configuredAdminEmail)
        : null;
    primaryAdmin = primaryAdmin || admins[0];

    const duplicateAdminIds = admins
        .filter(user => user._id.toString() !== primaryAdmin._id.toString())
        .map(user => user._id);

    if (duplicateAdminIds.length) {
        await User.updateMany({ _id: { $in: duplicateAdminIds } }, { $set: { role: 'teacher' } });
        console.warn(`⚠️ ${duplicateAdminIds.length} extra admin account(s) were demoted to teacher. Primary admin: ${primaryAdmin.email}`);
    }
}

databaseConnection.then(async () => {
    await Course.updateMany({ status: { $exists: false } }, { $set: { status: 'published' } }).catch(() => {});
    await enforceSingleAdmin().catch(err => console.error('Single-admin enforcement error:', err.message));
});
...
function requireRole(...roles) {
    return async (req, res, next) => {
        try {
            // Always read the current role from the database so an old session
            // cannot keep admin access after the account has been demoted.
            const user = await User.findById(req.session.userId).select('role name').lean();
            if (!user) {
                req.session.destroy(() => {});
...
app.post('/admin/users/create', requireLogin, requireRole('admin'), async (req,res)=>{
    try {
        ...
        // The Users page may create only Student or Teacher accounts.
        const role=['student','teacher'].includes(req.body.role) ? req.body.role : 'student';
        if(name.length<2||!email||password.length<6) return res.redirect('/admin/users?error=Invalid%20user%20details');
        const exists=await User.findOne({$or:[{email},{name}]});
        if(exists) return res.redirect('/admin/users?error=Email%20or%20username%20already%20exists');
        await User.create({name,email,password:await bcrypt.hash(password,10),role,emailVerified:true});
        res.redirect('/admin/users?success=User%20created');
    } catch(e){console.error(e);res.redirect('/admin/users?error=Could%20not%20create%20user');}
});
...
app.post('/admin/users/:id/role', requireLogin, requireRole('admin'), async (req, res) => {
    try {
        const { role } = req.body;
        // Admin cannot be assigned here. There is only one protected admin account.
        ...
        if (!validRoles.includes(role)) return res.redirect('/admin/users?error=Admin%20role%20cannot%20be%20assigned');
        await User.findByIdAndUpdate(req.params.id, { role });
        res.redirect('/admin/users?success=User%20role%20updated');
    } catch (err) {
        console.error('Change role error:', err);
        res.redirect('/admin/users?error=Could%20not%20update%20role');
    }
});
